...
- `貸出準備中` は満室扱い（×）
- `駐車場` UNIT は対象外
- 種別に「JR」が付く部屋（例: `5.0帖 JR`）は無いものとして扱う（表・件数・検算のすべてから除外）
- 種別の「中通路」は無視し、同じ帖数のタイプに統合する（例: `1.5帖 中通路` → `1.5帖`）
- 「変形」は室番号・種別・備考・メモのいずれかに含まれていれば変形部屋として NO 欄に表示する。
  ファボックの備考欄に「変形」と入れれば HP にも反映される
